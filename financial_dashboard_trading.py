...
def run_macd_backtest(df, fast_p, slow_p, signal_p, stop_loss, qty):
    ...
    hist = np.asarray(res[2] if isinstance(res, (list, tuple)) else res['macdhist'], dtype=np.float64)
    for n in range(1, len(df) - 1):
...
def run_kdj_backtest(df, fastk_p, slowk_p, slowd_p, overbought, oversold, stop_loss, qty):
    rec = Record()
    df = df.copy().reset_index(drop=True)
    res = STOCH(df.to_dict('series'), fastk_period=fastk_p, slowk_period=slowk_p,
                slowk_matype=0, slowd_period=slowd_p, slowd_matype=0)
    slowk = np.asarray(res[0] if isinstance(res, (list, tuple)) else res['slowk'], dtype=np.float64)
    slowd = np.asarray(res[1] if isinstance(res, (list, tuple)) else res['slowd'], dtype=np.float64)
    for n in range(1, len(df) - 1):
        if np.isnan(slowk[n-1]) or np.isnan(slowk[n]):
            continue
        if rec.GetOpenInterest() == 0:
            if slowk[n-1] <= slowd[n-1] and slowk[n] > slowd[n] and slowk[n] < oversold:
                rec.Order('Buy', df['product'][n+1], df['time'][n+1], df['open'][n+1], qty)
                sl = df['open'][n+1] - stop_loss
                continue
            if slowk[n-1] >= slowd[n-1] and slowk[n] < slowd[n] and slowk[n] > overbought:
                rec.Order('Sell', df['product'][n+1], df['time'][n+1], df['open'][n+1], qty)
                sl = df['open'][n+1] + stop_loss
                continue
        elif rec.GetOpenInterest() > 0:
...
opt_strategy = st.selectbox('選擇要最佳化的策略', ['MA 移動平均線', 'MACD', 'KDJ'], key='opt_strategy')
...
elif opt_strategy == 'KDJ':
    with st.expander("設定 KDJ 最佳化參數範圍"):
        kdj_fastk_range = st.slider('RSV 週期範圍（fastk_period）', 3, 30, (5, 14), key='opt_kdj_fastk')
        kdj_slowk_range = st.slider('K 平滑週期範圍（slowk_period）', 1, 10, (2, 5), key='opt_kdj_slowk')
        kdj_slowd_range = st.slider('D 平滑週期範圍（slowd_period）', 1, 10, (2, 5), key='opt_kdj_slowd')
        kdj_ob_range = st.slider('超買線範圍', 60, 95, (70, 85), key='opt_kdj_ob')
        kdj_os_range = st.slider('超賣線範圍', 5, 40, (15, 30), key='opt_kdj_os')
        kdj_sl_range = st.slider('停損範圍', 5, 100, (10, 50), key='opt_kdj_sl')
        kdj_step = st.number_input('步長', min_value=1, max_value=10, value=3, key='opt_kdj_step')

    if st.button('執行 KDJ 最佳化', key='run_kdj_opt'):
        results = []
        fk_vals = range(kdj_fastk_range[0], kdj_fastk_range[1]+1, int(kdj_step))
        sk_vals = range(kdj_slowk_range[0], kdj_slowk_range[1]+1, int(kdj_step))
        sd_vals = range(kdj_slowd_range[0], kdj_slowd_range[1]+1, int(kdj_step))
        ob_vals = range(kdj_ob_range[0], kdj_ob_range[1]+1, int(kdj_step))
        os_vals = range(kdj_os_range[0], kdj_os_range[1]+1, int(kdj_step))
        sl_vals = range(kdj_sl_range[0], kdj_sl_range[1]+1, int(kdj_step))
        combos = list(itertools.product(fk_vals, sk_vals, sd_vals, ob_vals, os_vals, sl_vals))
        prog = st.progress(0, text='KDJ 最佳化進行中...')
        for i, (fk, sk, sd, ob, os_, sl) in enumerate(combos):
            if os_ >= ob:
                prog.progress(min((i+1)/max(len(combos),1), 1.0))
                continue
            rec = run_kdj_backtest(KBar_df, fk, sk, sd, ob, os_, sl, 1)
            tp = rec.GetTotalProfit()
            mdd = rec.GetMDD()
            wr = rec.GetWinRate()
            rr = tp / mdd if mdd > 0 else 0
            results.append({
                'RSV週期': fk, 'K平滑': sk, 'D平滑': sd,
                '超買線': ob, '超賣線': os_, '停損': sl,
                '總盈虧': round(tp, 0), '勝率': round(wr, 3),
                'MDD': round(mdd, 0), '報酬風險比': round(rr, 3)
            })
            prog.progress(min((i+1)/max(len(combos),1), 1.0), text=f'KDJ 最佳化進行中... {i+1}/{len(combos)}')
        if results:
            opt_df = pd.DataFrame(results).sort_values('報酬風險比', ascending=False).reset_index(drop=True)
            st.write("**KDJ 最佳化結果（依報酬風險比排序）：**")
            st.dataframe(opt_df.head(20))
        else:
            st.write('無有效參數組合（請確認超賣線 < 超買線）')


#%%
####### (八) 策略比較總表 #######
st.subheader("(八) 策略比較總表")

def get_perf_row(name, rec):
    if len(rec.Profit) == 0:
        return {
            '策略': name, '交易次數': 0, '總盈虧(元)': 'N/A', '平均每次盈虧(元)': 'N/A',
            '勝率': 'N/A', '最大連續虧損(元)': 'N/A', 'MDD(元)': 'N/A', '報酬風險比': 'N/A'
        }
    tp = rec.GetTotalProfit()
    mdd = rec.GetMDD()
    return {
        ...
        '總盈虧(元)': f"{tp:,.0f}",
        '平均每次盈虧(元)': f"{rec.GetAverageProfit():,.0f}",
        '勝率': f"{rec.GetWinRate():.1%}",
        '最大連續虧損(元)': f"{rec.GetAccLoss():,.0f}",
        'MDD(元)': f"{mdd:,.0f}",
        '報酬風險比': f"{tp/mdd:.2f}" if mdd > 0 else 'N/A'
    }

compare_df = pd.DataFrame([
    get_perf_row('MA 移動平均線', OrderRecord),
    get_perf_row('MACD', OrderRecord_MACD),
    get_perf_row('KDJ', OrderRecord_KDJ),
])
compare_df = compare_df.set_index('策略')
...
def highlight_best(col):
    ...
    positive_best = col.name in ['總盈虧(元)', '平均每次盈虧(元)', '勝率', '報酬風險比', '交易次數']
    negative_best = col.name in ['最大連續虧損(元)', 'MDD(元)']
    ...
    styles[best_i] = 'background-color: #1a472a; color: white; font-weight: bold'
    return styles

st.write("**各策略績效比較（綠色底色 = 該欄最佳值）**")
st.dataframe(compare_df.style.apply(highlight_best, axis=0), use_container_width=True)


#%%
####### (九) 生成式 AI 自動評估策略績效 #######
st.subheader("(九) 生成式 AI 自動評估策略績效")
